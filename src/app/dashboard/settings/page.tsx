        ...
        <div className="max-w-6xl mx-auto px-6 pb-24 pt-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col lg:flex-row gap-10">
                <aside className="lg:w-64 shrink-0">
                    <nav className="flex flex-row lg:flex-col gap-2 sticky top-24 overflow-x-auto pb-4 lg:pb-0">
...
function TabButton({ active, onClick, icon: Icon, label, danger }: { active: boolean, onClick: () => void, icon: any, label: string, danger?: boolean }) { return ( <button onClick={onClick} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all w-auto lg:w-full shrink-0 text-left border ${active ? (danger ? "bg-rose-500/10 border-rose-500/20 text-rose-500" : "bg-indigo-500/10 border-indigo-500/20 text-indigo-400") : "bg-transparent border-transparent text-muted-foreground hover:bg-secondary/50"}`}><Icon className="size-4 shrink-0" /><span className="whitespace-nowrap">{label}</span></button> ) }

import pkg from 'pg';
const { Client } = pkg;
import * as dotenv from 'dotenv';
dotenv.config();

const client = new Client({
  connectionString: process.env.DATABASE_URL,
});

async function main() {
  console.log('Connecting to database...');
  await client.connect();

  console.log('Creating product-images bucket and RLS policies...');

  try {
    // 1. Create the bucket
    await client.query(`
      INSERT INTO storage.buckets (id, name, public)
      VALUES ('product-images', 'product-images', true)
      ON CONFLICT (id) DO NOTHING;
    `);
    console.log('Bucket "product-images" ensured.');

    // 2. Create RLS Policy for public read
    await client.query(`
      CREATE POLICY "Public Access"
      ON storage.objects FOR SELECT
      USING ( bucket_id = 'product-images' );
    `).catch(e => {
        if (!e.message.includes('already exists')) console.error('Policy warning:', e.message);
    });
    console.log('Public read policy ensured.');

    // 3. Create RLS Policy for authenticated inserts
    await client.query(`
      CREATE POLICY "Auth Insert"
      ON storage.objects FOR INSERT
      TO authenticated
      WITH CHECK ( bucket_id = 'product-images' );
    `).catch(e => {
        if (!e.message.includes('already exists')) console.error('Policy warning:', e.message);
    });
    console.log('Authenticated insert policy ensured.');

    // 4. Update policy
    await client.query(`
      CREATE POLICY "Auth Update"
      ON storage.objects FOR UPDATE
      TO authenticated
      USING ( bucket_id = 'product-images' );
    `).catch(e => {
        if (!e.message.includes('already exists')) console.error('Policy warning:', e.message);
    });
    console.log('Authenticated update policy ensured.');

    console.log('Successfully set up storage bucket and policies!');
  } catch (error) {
    console.error('Failed to create bucket:', error);
  } finally {
    await client.end();
  }
}

main();
